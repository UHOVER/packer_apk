...
function removeIconDir() {
    ...
    shelljs_1.exec(`rm -r ${sourceDir}/${iconFolder}`);
}
function makeIconDir() {
    console.log('创建icon文件夹');
    shelljs_1.exec(`mkdir ${sourceDir}/${iconFolder}`);
    iconSubFolders.forEach(f => {
        shelljs_1.exec(`mkdir ${sourceDir}/${iconFolder}/${f}`);
    });
}
function makeIcon(name) {
    return new Promise(resolve => {
        //拷贝icon到工具工程目录
        shelljs_1.exec(`cp ${sourceDir}/${name} ./`);
        ...
        for (let i = 0; i < sizes.length; i++) {
            ...
            if (i !== sizes.length - 1) {
                sharp(name).resize(s, s).overlayWith(roundedCorners, { cutout: true })
                    .toFile(`${sourceDir}/${iconFolder}/${iconSubFolders[i]}/${iconName}`, function (err) {
                    // console.log('回调' + s)
                    count++;
                    check();
                });
                ...
                sharp(name).resize(s, s).overlayWith(roundedCorners, { cutout: true })
                    .toFile(`${sourceDir}/${iconFolder}/${iconName}`, function (err) {
                    // console.log('回调' + s)
                    count++;
                    check();
                });
...
function build() {
    console.log('-------打包-------');
    shelljs_1.exec(`rm ${workDir}/build/ooutputs/apk/release/GAME-release.apk`);
    shelljs_1.exec(`cd ${workDir} && gradle assembleRelease`);
}
function renameApk(name) {
